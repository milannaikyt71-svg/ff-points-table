...
export default function Home() {
  ...
  const addResult = () => {
    if (!teamName) return; // require team name
    ...
    <main className="min-h-screen bg-black text-white p-6">
      ...
      <div className="bg-gray-900 p-6 rounded-xl mb-6">
        ...
        <input
          ...
          value={elims}
          onChange={(e) => setElims(Number(e.target.value))}
          className="w-full p-2 mb-3 text-black rounded"
        />

        <input
          type="number"
          placeholder="Rank (1-12)"
          value={rank}
        ...
        <button
          onClick={addResult}
          className="bg-yellow-500 px-4 py-2 rounded font-bold text-black"
        >
          Add Result
        </button>
      </div>
      ...
      <div className="bg-gray-900 p-6 rounded-xl">
        ...
        {mvp && (
          <div className="mt-6 p-4 bg-yellow-600 rounded text-black font-bold">
            MVP: {mvp.name} ({mvp.eliminations} Eliminations)
          </div>
        )}
      </div>
    </main>
